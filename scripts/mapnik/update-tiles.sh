...
if [ $? -eq 0 ]; then
	...
	${DIR}scale-tiles-to-lower-zoom-levels.py ${tiles_dir}/ $zoom_level > /dev/null

	if [ $? -eq 0 ]; then
		echo "$(current_time) Creating symlink..."
		ln -sfn $tiles_dir ${tiles_root_dir}${tiles_dir_base_name}-current
